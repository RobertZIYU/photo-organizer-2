...
interface ElectronAPI {
  ...
  testMainToRenderer: () => Promise<string>;
  onMainMessage: (callback: (message: string) => void) => void;
  selectFolder: () => Promise<string | null>;
  scanPhotos: (folderPath: string) => Promise<any>;
...
const PhotoOrganizeDemo = () => {
  ...
  const [previewStructure, setPreviewStructure] = useState<any>({});
  ...
  const [pendingPhotoForNewFolder, setPendingPhotoForNewFolder] = useState<any>(null);
  const [aiProgress, setAiProgress] = useState(0);
  const [currentStage, setCurrentStage] = useState('analyzing'); // 'analyzing' or 'reorganizing'

  // Real photo data from backend
  ...
  useEffect(() => {
    if (window.electronAPI) {
      if (window.electronAPI.onScanProgress) {
        window.electronAPI.onScanProgress((progress: any) => {
  ...
  const handleFolderSelect = async () => {
    try {
      // Use real Electron folder selection
      ...
      if (folder) {
        ...
        const result = await window.electronAPI.scanPhotos(folder);

        if (result && result.photos) {
          setRealPhotos(result.photos);
      ...
      console.error('Error selecting/scanning folder:', error);
      setIsProcessing(false);
  ...
  const startOrganizing = async () => {
    ...
    try {
      const photosToUse = realPhotos.length > 0 ? realPhotos : mockPhotos;

      // Stage 1: Generate thumbnails (0-30%)
      const photosWithThumbs = await Promise.all(
        photosToUse.map(async (photo, index) => {
          if (photo.path) {
            try {
              const thumb = await window.electronAPI.generateThumbnail(photo.path, 200, 150);
              setAiProgress(Math.round(((index + 1) / photosToUse.length) * 30));
              return { ...photo, thumb };
            } catch (error) {
              console.error(`Failed to generate thumbnail for ${photo.path}:`, error);
              return { ...photo, thumb: 'https://picsum.photos/200/150?random=1' };
            }
          } else {
            return photo;
          }
      ...
      // Stage 2: AI Analysis (30-80%)
      setAiProgress(30);
      if (realPhotos.length > 0 && window.electronAPI.analyzePhotos) {
        try {
          const photoPaths = photosWithThumbs
            .filter(p => p.path)
            .map(p => p.path);

          if (photoPaths.length > 0) {
            const aiResults = await window.electronAPI.analyzePhotos(photoPaths);

            // Merge AI results back into photos
            photosWithThumbs.forEach(photo => {
              if (photo.path && aiResults[photo.path]) {
                photo.aiData = aiResults[photo.path];
              }
            });
          }
        } catch (error) {
      ...
      // Stage 3: Organize (80-100%)
      setAiProgress(80);
      setCurrentStage('reorganizing');

      // Use real organization engine
      let structure: any = {};
      if (window.electronAPI && window.electronAPI.organizePhotos) {
        try {
          const organizationResult = await window.electronAPI.organizePhotos(customQuery, photosWithThumbs);
          structure = organizationResult.folderStructure;
          console.log('Organization result:', organizationResult);
        } catch (error) {
          console.error('Organization failed, using fallback:', error);
          structure = generateInitialStructure(photosWithThumbs);
        }
      } else {
        // Fallback to client-side organization
        structure = generateInitialStructure(photosWithThumbs);
      }
  ...
  const generateInitialStructure = (photos: any[] = mockPhotos) => {
    ...
    if (customQuery.toLowerCase().includes('date') || customQuery.toLowerCase().includes('time') || customQuery.toLowerCase().includes('year/month/day')) {
      // Organize by date from EXIF data
      const byDate: any = {};
      photos.forEach(photo => {
        const dateKey = photo.exifData?.dateTaken
          ? new Date(photo.exifData.dateTaken).toISOString().split('T')[0]
          : (photo.date || 'Unknown_Date');
      ...
      photos.forEach(photo => {
        const locationKey = photo.exifData?.gpsLocation
          ? `GPS_${photo.exifData.gpsLocation.latitude.toFixed(2)}_${photo.exifData.gpsLocation.longitude.toFixed(2)}`
          : (photo.location || 'Unknown_Location');
        if (!byLocation[locationKey]) byLocation[locationKey] = [];
        byLocation[locationKey].push(photo);
      });
      return byLocation;
    } else if (customQuery.toLowerCase().includes('people') || customQuery.toLowerCase().includes('face')) {
      // For now, use mock organization for people (AI face recognition will be Phase 3)
      return {
        'People_Group_1': photos.slice(0, Math.ceil(photos.length / 2)),
        'People_Group_2': photos.slice(Math.ceil(photos.length / 2))
      };
    } else if (customQuery.toLowerCase().includes('month')) {
      // Organize by month
      const byMonth: any = {};
      photos.forEach(photo => {
        const date = photo.exifData?.dateTaken ? new Date(photo.exifData.dateTaken) : (photo.date ? new Date(photo.date) : null);
        const monthKey = date ? `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}` : 'Unknown_Month';
        if (!byMonth[monthKey]) byMonth[monthKey] = [];
        byMonth[monthKey].push(photo);
      });
      return byMonth;
    } else if (customQuery.toLowerCase().includes('camera')) {
      // Organize by camera model
      const byCamera: any = {};
      photos.forEach(photo => {
        const cameraKey = photo.exifData?.camera || 'Unknown_Camera';
        if (!byCamera[cameraKey]) byCamera[cameraKey] = [];
        byCamera[cameraKey].push(photo);
      });
      return byCamera;
    } else {
      // Default custom organization - group by halves
      return {
        'Group_1': photos.slice(0, Math.ceil(photos.length / 2)),
        'Group_2': photos.slice(Math.ceil(photos.length / 2))
      };
    }
  };

  const handleCustomQuery = (e: React.KeyboardEvent) => {
  ...
  const runAIWithCorrections = () => {
    ...
    setTimeout(() => {
      ...
      Object.keys(newStructure).forEach(key => {
        if ((newStructure as any)[key].length === 0) {
          delete (newStructure as any)[key];
        }
      });
  ...
  const handleDragStart = (photo: any, fromFolder: string) => {
  ...
  const handleDrop = (e: React.DragEvent, toFolder: string) => {
    e.preventDefault();
    setIsDraggingOverNewFolder(false);
    ...
    const newStructure = { ...previewStructure };
    newStructure[fromFolder] = newStructure[fromFolder].filter((p: any) => p.name !== photo.name);
  ...
  const renderWorkspace = () => {
    return (
      <div className="space-y-4">
        {Object.entries(previewStructure).map(([folderName, photos]: [string, any]) => (
          <div
            key={folderName}
          ...
          {isProcessing && scanProgress ? (
            <div className="bg-white rounded-xl shadow-sm p-12 text-center mb-8">
              ...
              <h2 className="text-xl font-semibold text-gray-900 mb-2">
                {scanProgress.stage === 'scanning' ? 'Scanning Folder...' : 'Processing Photos...'}
              </h2>
              <p className="text-gray-500 mb-6">
                {scanProgress.stage === 'scanning'
                  ? 'Finding all photos in the folder...'
                  : `Processing ${scanProgress.currentFile || 'photos'}...`}
              </p>
          ...
          <div className="bg-white rounded-lg p-6 mb-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">{photoCount || 5} photos found</p>
              </div>
              <button
                onClick={() => {
                  setSelectedFolder(null);
                            ...
                            const stage2Interval = setInterval(() => {
                              ...
                              if (progress >= 100) {
                                ...
                                setTimeout(async () => {
                                  ...
                                  // Use real organization engine for re-organization
                                  let newStructure: any = {};
                                  if (window.electronAPI && window.electronAPI.organizePhotos) {
                                    try {
                                      const organizationResult = await window.electronAPI.organizePhotos(
                                        customQuery + ' (refined based on user changes)',
                                        allPhotos
                                      );
                                      newStructure = organizationResult.folderStructure;
                                    } catch (error) {
                                      console.error('Re-organization failed, using fallback:', error);
                                      newStructure = {
